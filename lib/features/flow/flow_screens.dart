...
import '../../navi/speech_service.dart';
import '../../navi/intent_parser.dart';
import '../../navi/navigation_controller.dart';
import '../../navi/tts_service.dart';
    ...
    child: AppCard(
      child: ListTile(
        leading: CircleAvatar(
          backgroundColor: AppColors.cyan.withOpacity(.15),
            ...
            child: Column(
              children: [
                ...
                ScaleTransition(
                  ...
                  child: GestureDetector(
                    onTap: () {
                      Navigator.pushNamed(c, '/listening');
                    },
                    child: const GlowOrb(
                      icon: Icons.mic_rounded,
                      size: 172,
                ...
                const Text(
                  'Tap and hold the microphone to talk',
...
class _ListeningScreenState extends State<ListeningScreen> {
  ...
  Future<void> _startListening() async {
    ...
    if (!ready) {
      await _tts.speak("Speech recognition is unavailable right now.");
      if (mounted) Navigator.pop(context);
      return;
    }
    ...
    // Audio cue so the user knows exactly when to speak
    await _tts.speak("Go ahead");

    if (!_isActive) return;

    final spokenText = await _speechService.listenOnce();
    ...
    setState(() {
      _statusText = 'You said: "$spokenText"';
      _isProcessing = true;
    });

    final destination = IntentParser.extractNavigationDestination(spokenText);

    if (destination != null) {
      if (mounted) Navigator.pop(context);
      await _navController.startNavigation(destination);
    } else {
      await _tts.speak("Sorry, I didn't understand that. Please try again.");
      if (_isActive) _startListening();
    }
  }
  ...
  Widget build(BuildContext c) => AppPage(
    child: Column(
      children: [
        ...
        const Spacer(),
        GlowOrb(icon: Icons.mic_rounded, size: 150, active: !_isProcessing),
