...
class HomeScreen extends StatelessWidget { const HomeScreen({super.key}); @override Widget build(BuildContext c)=>const SpeakScreen(); }

class ReadScreen extends StatelessWidget { const ReadScreen({super.key}); @override Widget build(BuildContext c)=>AppPage(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const SizedBox(height:8),const Row(children:[AppBackButton(),Spacer(),Text('Read Mode',style:TextStyle(fontWeight:FontWeight.w800)),SizedBox(width:45)]),const SizedBox(height:14),AppCard(child:SizedBox(height:255,width:double.infinity,child:Stack(alignment:Alignment.center,children:[Container(margin:const EdgeInsets.all(18),decoration:BoxDecoration(borderRadius:BorderRadius.circular(14),border:Border.all(color:AppColors.cyan,width:2)),),const Icon(Icons.document_scanner_outlined,size:80,color:AppColors.cyan),const Positioned(bottom:30,child:Chip(label:const Text('Detected text'),backgroundColor:AppColors.surfaceLight))]))),const SizedBox(height:13),const AppCard(child:ListTile(leading:Icon(Icons.auto_awesome,color:AppColors.cyan),title:Text('DETECTED TEXT'),subtitle:Text('Platform 5, Gate 3 • Central Station. Boarding in 4 minutes.'))),const SizedBox(height:13),Row(children:[_RoundAction(Icons.play_arrow_rounded,'Listen'),_RoundAction(Icons.copy_rounded,'Copy'),_RoundAction(Icons.translate_rounded,'Translate'),_RoundAction(Icons.share_outlined,'Share')]),const Spacer(),PrimaryButton(label:'Scan text',icon:Icons.camera_alt_rounded,onPressed:()=>Navigator.pushNamed(c,'/processing')),const SizedBox(height:16)])); }
  ...
  Widget build(BuildContext context) => Scaffold(
        ...
        body: Container(
          decoration: const BoxDecoration(
            gradient: RadialGradient(radius: .9, colors: [Color(0xFF8B203C), Color(0xFF210E20)]),
          ),
          child: SafeArea(
            child: Padding(
              ...
              child: Column(children: [
                ...
                const Text('EMERGENCY SOS', style: TextStyle(color: Color(0xFFFFABBB), fontWeight: FontWeight.bold, letterSpacing: 1.5)),
                const SizedBox(height: 10),
                const Text('Hold to send SOS', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
                const Text('Alert your emergency contacts', style: TextStyle(color: Color(0xFFFFB9C5))),
                const Spacer(),
                Container(
                  width: 208, height: 208,
                  decoration: const BoxDecoration(shape: BoxShape.circle, color: AppColors.danger, boxShadow: [BoxShadow(color: Colors.red, blurRadius: 48)]),
                  child: Center(child: Text('$seconds\nSOS', textAlign: TextAlign.center, style: const TextStyle(fontSize: 38, fontWeight: FontWeight.w900))),
                ),
                const SizedBox(height: 20),
                Text('Sending location in $seconds seconds', style: const TextStyle(color: Color(0xFFFFBAC6))),
                const Spacer(),
                SizedBox(
                  width: double.infinity, height: 52,
                  child: OutlinedButton.icon(
                    style: OutlinedButton.styleFrom(foregroundColor: Colors.white, side: const BorderSide(color: Color(0xFFB64F65))),
                    onPressed: () => Navigator.pop(context), icon: const Icon(Icons.close), label: const Text('Cancel'),
                  ),
                ),
                const SizedBox(height: 16),
              ]),
...
class OfflineScreen extends StatelessWidget { const OfflineScreen({super.key}); @override Widget build(BuildContext c)=>AppPage(child:Center(child:Column(mainAxisSize:MainAxisSize.min,children:[const GlowOrb(icon:Icons.cloud_off_rounded,size:140),const SizedBox(height:27),const Text("You're offline",style:TextStyle(fontSize:27,fontWeight:FontWeight.w900)),const SizedBox(height:10),const Text('Location and saved routes still work. Navigation and travel need internet.',textAlign:TextAlign.center,style:TextStyle(color:AppColors.muted)),const SizedBox(height:28),PrimaryButton(label:'Try again',icon:Icons.refresh,onPressed:()=>Navigator.pushNamedAndRemoveUntil(c,'/home',(_)=>false)),TextButton.icon(onPressed:()=>Navigator.pushNamedAndRemoveUntil(c,'/home',(_)=>false),icon:const Icon(Icons.shield_outlined),label:const Text('Use offline features'))]))); }

class ArrivedScreen extends StatelessWidget { const ArrivedScreen({super.key}); @override Widget build(BuildContext c)=>AppPage(child:Column(children:[const SizedBox(height:38),const GlowOrb(icon:Icons.check_rounded,size:150,active:true),const SizedBox(height:26),const Text("You've arrived",style:TextStyle(fontSize:27,fontWeight:FontWeight.w900)),const SizedBox(height:8),const Text('Central Library · 2 min remaining to entrance',textAlign:TextAlign.center,style:TextStyle(color:AppColors.muted)),const SizedBox(height:22),const AppCard(child:Padding(padding:EdgeInsets.all(14),child:Row(mainAxisAlignment:MainAxisAlignment.spaceAround,children:[_Arrival('5:42','Arrival'),_Arrival('0.4 km','Distance'),_Arrival('7 min','Elapsed')])),),const Spacer(),PrimaryButton(label:'Back to home',icon:Icons.home_rounded,onPressed:()=>Navigator.pushNamedAndRemoveUntil(c,'/home',(_)=>false)),const SizedBox(height:10),SizedBox(width:double.infinity,height:49,child:OutlinedButton.icon(style:OutlinedButton.styleFrom(side:const BorderSide(color:AppColors.outline)),onPressed:(){},icon:const Icon(Icons.bookmark_outline),label:const Text('Save this route'))),const SizedBox(height:16)])); }
class _Arrival extends StatelessWidget {final String value,label;const _Arrival(this.value,this.label);@override Widget build(BuildContext c)=>Column(children:[Text(value,style:const TextStyle(color:AppColors.cyan,fontWeight:FontWeight.w900)),Text(label,style:const TextStyle(color:AppColors.muted,fontSize:10))]);}

class MicrointeractionsScreen extends StatelessWidget { const MicrointeractionsScreen({super.key}); @override Widget build(BuildContext c)=>AppPage(child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const SizedBox(height:18),const Text('Microinteractions',style:TextStyle(fontSize:25,fontWeight:FontWeight.w900)),const SizedBox(height:22),GridView.count(shrinkWrap:true,crossAxisCount:2,crossAxisSpacing:12,mainAxisSpacing:12,childAspectRatio:1.05,children:const[_Mini(Icons.mic_rounded,'Voice pulse'),_Mini(Icons.graphic_eq_rounded,'Listening'),_Mini(Icons.circle,'Loading'),_Mini(Icons.vibration_rounded,'Haptics'),_Mini(Icons.more_horiz,'Progress dots'),_Mini(Icons.toggle_on_rounded,'Button glow')]),const SizedBox(height:14),const AppCard(child:ListTile(title:Text('Every interaction should feel simple + confident'),subtitle:Text('Soft motion, meaningful feedback, and clear voice cues.')))])); }
class _Mini extends StatelessWidget {final IconData icon;final String label;const _Mini(this.icon,this.label);@override Widget build(BuildContext c)=>AppCard(child:Column(mainAxisAlignment:MainAxisAlignment.center,children:[GlowOrb(icon:icon,size:55),const SizedBox(height:10),Text(label,style:const TextStyle(fontSize:11))]));}
